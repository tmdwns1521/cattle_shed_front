...
export default {
  ...
  methods: {
    ...
    async addData() {
      ...
      await this.$axios.post('http://localhost:3001/api/createNew', data);
    ...
    async updateData() {
      ...
      await this.$axios.post('http://localhost:3001/api/updateNew', data);
      location.reload();
    ...
    async getCurrentMonthsalesData() {
      const data = await this.$axios.get(
        "http://localhost:3001/api/getCattleShed"
      );
      this.onList(data);
    },
    async getSalesData() {
